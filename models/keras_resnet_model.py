...
def create_keras_resnet_model(input_shape, num_classes):
...
def train_keras_resnet(dataset_path, num_epochs=10):
    img_size = (224, 224)
    batch_size = 32
    ...
    model = create_keras_resnet_model((*img_size, 3), num_classes)
    ...
    plt.ylabel('True')
    plt.savefig('keras_resnet_confusion_matrix.png')
    plt.close()

    # Save model
    model.save('keras_resnet_model.h5')
    print("Keras ResNet model saved.")

    return history

if __name__ == "__main__":
    dataset_path = os.path.join(os.path.dirname(__file__), "..", "dataset", "TRAIN")
    train_keras_resnet(dataset_path, num_epochs=10)