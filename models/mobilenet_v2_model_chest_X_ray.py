...
def train_model(model, train_loader, test_loader, class_names, num_epochs=5):
    ...
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.savefig('mobilenet_v2_accuracy.png')
    plt.close()
    ...
    if len(class_names) == 2:
        ...
        plt.savefig('mobilenet_v2_roc_curve.png')
        plt.close()

    return train_losses, test_losses, train_accs, test_accs

def train_mobilenet_v2(dataset_path, num_epochs=2):
    ...
    torch.save(mobilenet.state_dict(), os.path.join('trained_models', 'mobilenet_v2_model(chest_X_ray).pth'))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        num_epochs = int(sys.argv[2]) if len(sys.argv) > 2 else 2
    else:
        dataset_path = "dataset/TRAIN"  # Adjust path as needed
        num_epochs = 5
    train_mobilenet_v2(dataset_path, num_epochs)